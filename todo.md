- https://es.wikipedia.org/wiki/Costum_de_Valencia
- We should allow for deletions.
- How it's revenew recogniized using accounting?
- Does the term token economy makes sense?
- What underlying epistemic artifact informs the idea that a nation producing potatoes can be just as rich as a nation producing potato chips?
- How to think about debt in the national currency?
-  Government Debt (in National Currency) as Pseudo-Debt   (Debt  ->  Mechanism; Goal - Tool to Managemnt the Monetary System) -> Monetary System Debt Tool -> Cuasideuda -> Quasi-debt -> Quasiobligation
-  Government Debt (in Foreign Currency)   (Debt) (Obligation)
-  Shoudl the https://github.com/csiglab/research-CountryIndex - project be integrated as a sub project of Socionogis.
  -  Should  the dominican republic 1.0 be in? Shold the two be?
- Cattle Auction System
  - https://mylivestock.co.nz/go/home/index
  - https://www.royalfloraholland.com/Entidade
  - Cargill
  - Bunge
  - Chicago Board of Trade
  - ...
- Market Price Tracker
- Credit Market
- Tax System
- Dominican Tax System
- Dominican Finantial Market
- Exchange Rate
  - How to model  Interest Rate?
- Interest Rate
  - How to model  Interest Rate?
- Money Market (https://en.wikipedia.org/wiki/Money_market)
- Relative Strengh Index  (Technical Indicator)
- Thessaloniki
- Patras
- Athens
- Cañada Real
- Rentabilida
- [Wang Xing](https://en.wikipedia.org/wiki/Wang_Xing)
- Groupon, Inc.
- Piamonte
- Mancomunidad
- KKR & Co. Inc.,

## Production 

-  How to reason about retail? What is the deep role of retail?
- [Taptools](https://www.taptools.io/)
- Asset toketization.
- Tokenization in livesstock production
- What problesm does 'Tokenization' solves?
- ...
- Tasar -> Unidad de Produccion -> Matrix Co-Produccion
- Serun Product
- Beuty & Personal Care Production - Market Chain
  ├── Skin Care
│ ├── Cleansers
│ ├── Toners
│ ├── Moisturizers
│ ├── Serums
│ ├── Face Oils
│ ├── Eye Creams
│ └── Sunscreen

- Food Security
  - https://en.wikipedia.org/wiki/Glufosinate
  - https://www.ewg.org/apps/
  - https://yuka.io/en/
  - https://www.ewg.org/foodnews/dirty-dozen.php
- https://food.ec.europa.eu/plants/pesticides/eu-pesticides-database_en?utm_source=chatgpt.com
- Stock Market
- Dominican Republic Land Market
  - https://inmueble.mercadolibre.com.do/MRD-747228712-terreno-de-227-tareas-con-mucha-naturaleza-en-anton-sanchez-_JM
  - 
- Visual Technical Object Set -> CAD Application, Parametric Modeling,  Constraint solver, Geometric Kernel, Computational Geometry, etc.
- Virtual Technical Production Object
- Bond Markat Tracket
- ...
